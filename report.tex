...
\begin{titlepage}
	\thispagestyle{empty}
    \begin{center}
        МГУ имени М.\,В.~Ломоносова\par\medskip Механико-математический факультет
...
\[
	\lim_{x \to a_k +}e^xf^{\prime}(x) = +\infty,\quad \lim_{x \to b_k -}e^xf^{\prime}(x) = -\infty.
\]
Значит, $f^{\prime}$ имеет на каждом интервале $I_k$ ровно один нуль.

Таким образом, на каждом из интервалов $I_k$, $k \in \Z$, функция $f$ сначала возрастает, а потом убывает, а значит, имеет не более двух корней на нём. Обозначим корни на интервалах $(a_k; 2\pi k)$ и $(2\pi k; b_k)$, через $x_k^{-}$ и $x_k^{+}$ соответственно.

\subsection{Корни уравнения $f(x) = 0$ на $I_k$, $k \leqslant -1$}
...
\subsection{Корни уравнения $f(x) = 0$ на $I_k$, $k \to \infty$}
...
\begin{proof}
	...
	\]
	Если $0 < v \leqslant t < \frac{\pi}{2}$, то $\frac{1}{\cos^2v} \leqslant \frac{1}{\cos^2t}$, следовательно, при $0 < u < t$
	\[
...
\]
Из $0 < \alpha_k < \frac{\pi}{2}$ и первого неравенства в \eqref{eq:L_estimates} вытекает, что при $k \geqslant 1$
\[
...
\]
для всех допустимых значений $\eps$. Поэтому для $k \leqslant -1$ в качестве корней уравнения $f(x) = 0$ на $I_k$ будут использоваться $2\pi k \pm \frac{\pi}{2}$. Отметим, что в этих точках функция $f$ не определена, поэтому проверять корни подстановкой некорректно.
...
\[
	\frac{3e^{-2\pi k}}{2\pi k - \frac{\pi}{2}} > \eps.
\]
В противном случае в качестве корней будем использовать величины $2\pi k \pm h_k$.

\section{Результаты работы программы}

Приведём результаты работы программы для нескольких значений относительной погрешности $\eps$. При $\eps = 10^{-2}$ программа вычисляет корни только для $k = 0$:
...
При $\eps = 10^{-5}$ программа вычисляет корни для $k = 0, 1$:
...
При $\eps = 10^{-8}$ программа вычисляет корни для $k = 0, 1, 2$:

\begin{center}
	\begin{tabular}{c|cc}
		$k$ & $x_k^-$               & $x_k^+$              \\[.5mm]
		\hline
		0   & $-1.5623145501348534$ & $0.8575493006219217$ \\
		1   & $6.2201352232447986$  & $6.3424958448101165$ \\
		2   & $12.563726126605642$  & $12.569008173732678$
	\end{tabular}
\end{center}

Начиная с $k = 3$, используется асимптотическая формула; её относительная погрешность не превосходит $1{.}14 \cdot 10^{-9} < 10^{-8}$.

При $\eps = 10^{-12}$ программа вычисляет корни для $k = 0, 1, 2, 3, 4$:
...
\begin{center}
	\begin{tabular}{c|cc}
		...
		2   & $12.56372615880127$  & $12.569008095226552$  \\
		3   & $18.849441788681808$  & $18.849670041389452$  \\
		4   & $25.13273629687863$  & $25.13274616055806$
	\end{tabular}
\end{center}

Начиная с $k = 5$, используется асимптотическая формула; её относительная погрешность не превосходит $2{.}29 \cdot 10^{-15} < 10^{-12}$.

\medskip
\hrule
\medskip

По ссылке \url{https://github.com/pshenikita/automatic-enigma} доступен актуальный исходный код этого отчёта и описанной программы.
